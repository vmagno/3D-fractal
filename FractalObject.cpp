    ...
    , MaxTriangles_(0)
{
    ...
    Param_.MinPosition = make_float3(-Param_.VoxelSize.x * Param_.VoxelGridSize.x / 2.f,
...
void FractalObject::Init(ShaderProgram* Shaders, TransformMatrix* Projection, TransformMatrix* Visualization,
                         ...
                         const uint3* /*Connectivity*/, const uint    NumElements, const float4* /*Colors*/,
                         const uint /*NumColors*/, const SDL_Surface* TexImage, const float2* TexCoord)
{
    MaxTriangles_      = NumElements;
    Param_.MaxVertices = MaxTriangles_ * 3;

    DisplayItem::Init(Shaders, Projection, Visualization, NULL, NULL, Param_.MaxVertices, NULL, NumElements, NULL,
                      Param_.MaxVertices, TexImage, TexCoord);
...
void FractalObject::Update()
{
...

    {
        ...
        CudaCheck(cudaMemcpy(&LastElement, DevicePointers_.VoxelVertices + Param_.NumVoxels - 1, sizeof(uint),
                             cudaMemcpyDeviceToHost));
        CudaCheck(cudaMemcpy(&LastScanElement, DevicePointers_.VoxelVerticesScan + Param_.NumVoxels - 1, sizeof(uint),
                             cudaMemcpyDeviceToHost));
    ...
    CudaCheck(
      cudaMemcpyToArray(TexArray_, 0, 0, DevicePointers_.TexCudaTarget, TexDataSize_, cudaMemcpyDeviceToDevice));
...
void FractalObject::InitTexture()
{
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, Param_.VoxelGridSize.x, Param_.VoxelGridSize.y, 0, GL_RGBA,
...
void FractalObject::MapBuffers()
{
    for (uint iBuffer = 0; iBuffer < NumVBO_; iBuffer++)
    {
        ...
        CudaCheck(
          cudaGraphicsResourceGetMappedPointer(GetArrayAddress(iBuffer), &NumBytes, CudaVboResources_[iBuffer]));
    }
...
void** FractalObject::GetArrayAddress(uint BufferIndex)
{
    switch (BufferIndex)
    {
    case VERTEX_VBO_ID: return (void**)&DevicePointers_.Vertices;
    case NORMAL_VBO_ID: return (void**)&DevicePointers_.Normals;
    case CONNECT_VBO_ID: return (void**)&DevicePointers_.Connectivity;
    case COLOR_VBO_ID: return (void**)&DevicePointers_.Colors;
    default: cerr << "[ERROR] Accessing a non-existing buffer (trying to map VBOs to CUDA resources)" << endl; break;
    }
...
void FractalObject::CreateTextures()
{
    ...
    CudaCheck(
      cudaMemcpy(DevicePointers_.NumVertexTable, NumVertexTable, sizeof(NumVertexTable), cudaMemcpyHostToDevice));
