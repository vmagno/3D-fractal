...
void SDLWindow::HandleKeyPress(SDL_Keycode Key, bool bPress)
{
    ...
    {
    case SDLK_ESCAPE:
    case SDLK_q: bDoContinue_ = false; break;
    case SDLK_w:
    case SDLK_UP: Camera_.SetMoveForward(bPress); break;
    case SDLK_a:
    case SDLK_LEFT: Camera_.SetMoveLeft(bPress); break;
    case SDLK_s:
    case SDLK_DOWN: Camera_.SetMoveBack(bPress); break;
    case SDLK_PAGEUP: Camera_.SetMoveUp(bPress); break;
...
void SDLWindow::HandleMouseClick(int Button, int State, int x, int y)
{
    ...
    default: cerr << "Unknown mouse button pressed" << endl; break;
    }
}
...
void SDLWindow::InitScene()
{
...

    {
        ...
        float TexCoord[] = {0.f, 1.f, 1.f, 1.f, 1.f, 0.f, 0.f, 0.f};

        HUD_->Init(&BaseShaders_, HUDProj, HUDVis, (float3*)Vertices, NULL, 4, (uint3*)Connect, 2, &Color, 1, TexImage,
                   (float2*)TexCoord);

        {
            Fractal_ = new FractalObject();
            Fractal_->Init(&PhongShader_, &ProjMatrix_, &VisMatrix_, NULL, NULL, 100000, NULL, 1500000, NULL, 100000,
                           NULL, NULL);
